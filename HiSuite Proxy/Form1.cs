...
namespace HiSuite_Proxy
{
    public partial class Form1 : Form
    {
        ...
        public bool Patch(string filename, bool systemadmin = false)
        {
            ...
            byte[] finddata = new byte[] { 0x6A, 0x00, 0x6A, 0x01, 0xFF, 0x35, 0x70, 0x68, 0x03, 0x10, 0xFF }, replacedata = new byte[] { 0x6A, 0x00, 0x6A, 0x00, 0xFF, 0x35, 0x70, 0x68, 0x03, 0x10, 0xFF };

            try
            {
                string settings = Environment.GetFolderPath(Environment.SpecialFolder.ProgramFilesX86) + @"\HiSuite\RunInfo.ini";
                if(File.ReadAllText(settings).Contains("version=11.0.0.510"))
                {
            ...
            {
                if (PatcherReplace(new byte[] { 0x71, 0x75, 0x65, 0x72 }, new byte[] { 0x70, 0x70, 0x70, 0x70 }, ref filedata))
                {
